...
foreach my $aa(@etc_all_file){
	...
	foreach my $le(@IN){
		...
		if ($Temp1[0]=~/^Gene\s/){
			...
			$hash_a{$Temp1[0]}=();
			push @{$hash_a{$Temp1[0]}},"$Temp1[1]";
			push @{$hash_a{$Temp1[0]}},"$Temp1[2]:$Temp1[4]-$Temp1[5]($Temp1[3])";
			######## if FPKM or TPM is 0 , the gene is 0.0001 #########
			if($Temp1[$select_E]<=0.000000001){
				######## if the gene have more than 2 vale , combine them #######
				if($hash_v{$aa}{$Temp1[0]}){
					if($hash_v{$aa}{$Temp1[0]}==0.0001){
						$hash_v{$aa}{$Temp1[0]}=0.0001;
					}else{
						$hash_v{$aa}{$Temp1[0]}=$hash_v{$aa}{$Temp1[0]}+$Temp1[$select_E];
					}
				}else{
					$hash_v{$aa}{$Temp1[0]}=0.0001;
				}
				...
				if($hash_v{$aa}{$Temp1[0]}){
					if($hash_v{$aa}{$Temp1[0]}==0.0001){
						$hash_v{$aa}{$Temp1[0]}=$Temp1[$select_E];
					}else{
						$hash_v{$aa}{$Temp1[0]}=$hash_v{$aa}{$Temp1[0]}+$Temp1[$select_E];
					}	
				}else{
...
foreach my $kk (sort keys (%hash_va)){
